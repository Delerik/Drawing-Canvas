...
class TestMyModule(unittest.TestCase):
    canvas_result1 = np.array([[' ' for x in range(int(4))] for y in range(int(20))])

    @mock.patch('domain.main.operations')
    def test_1(self, mocked):
        print('test 1')
        # mocked.return_value = self.canvas_result1
        val1 = str(create_canvas(4, 20))

        self.assertEqual(val1, str(self.canvas_result1))

    @mock.patch('domain.main.operations')
    def test_2(self, mocked):
        print('test 2')
        # mocked.return_value = self.canvas_result1
        val1 = create_line(self.canvas_result1, 1, 1, 1, 2)
    ...
    def test_3(self, mocked):
        print('test 3')
        # mocked.return_value = self.canvas_result1
        val1 = create_rectangle(self.canvas_result1, 0, 0, 2, 2)
        canvas = np.array([[' ' for x in range(int(4))] for y in range(int(20))])
        canvas[0, 0] = 'x'
        canvas[0, 1] = 'x'
        canvas[0, 2] = 'x'
    ...
    def test_4(self, mocked):
        print('test 4')
        # mocked.return_value = self.canvas_result1
        val1 = fill_area(self.canvas_result1, 0, 0, 'c')
